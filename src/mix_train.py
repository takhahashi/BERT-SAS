...
from utils.cfunctions import regvarloss, mix_loss3
    ...
    for epoch in range(cfg.training.n_epochs):
        ...
        for data in train_dataloader:
            ...
            with torch.cuda.amp.autocast():
                ...
                mse_loss, cross_loss, _ = mix_loss3(data['labels'].squeeze(), outputs['score'].squeeze(), outputs['logits'], high=upper_score, low=0)
                loss, s_wei, diff_wei, alpha, pre_loss = weight_d(cross_loss, mse_loss)
        ...
        for dev_data in dev_dataloader:
            ...
            #loss, s_wei, diff_wei, alpha, pre_loss = weight_d(crossentropy_el, mseloss_el)
            mse_loss, cross_loss, _ = mix_loss3(d_data['labels'].to('cpu').detach().squeeze(), dev_outputs['score'].squeeze(), dev_outputs['logits'], high=upper_score, low=0)
            loss, s_wei, diff_wei, alpha, pre_loss = weight_d(cross_loss, mse_loss)
        ...
        wandb.log({"epoch":epoch+0.001,"all_loss":lossall, 
